...
function apiCall(httpMethod, options, callback) {
  ...
  auth.getToken(options.config, function(error, token) {
    if (error) {
      ...
      var requestOptions = {
        method: httpMethod,
        url: urls.getBaseUrl() + options.path,
        json: true,
        headers: {
      ...
      logger('Request: ' + httpMethod + ' ' + urls.getBaseUrl() + options.path);
      ...
      request(requestOptions, function(error, response, responseBody) {
        ...
        if (error) {
          ...
          logger('API error: ' + responseBody.error);
          ...
          logger('API error: ' + responseBody.result.error.detail);
          ...
          logger('Success: ' + response.statusCode + ' ' + urls.getBaseUrl() + options.path);
