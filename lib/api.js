    ...
    if (error) {
      ...
      request(requestOptions, (err, { statusCode = 500, headers, request: req } = {}, responseBody) => {
        const meta = {
          ...
          response: {
            responseBody: processLogData(responseBody),
            statusCode,
            headers,
          },
        };
        if (err) {
          ...
          log(`API Error: ${processLogData(responseBody)}`, { level: 'error', meta });
          ...
          log(`API Error: ${processLogData(responseBody.result.error.detail)}`, { level: 'error', meta });
