...
function apiCall(httpMethod, options, callback) {
  ...
  auth.getToken(options.config, (error, token) => {
    if (error) {
      debugLogs.push({ function: 'auth.getToken', message: `Error: ${error}` });
      ...
      request(requestOptions, (err, response, responseBody) => {
        if (err) {
          debugLogs.push({ function: 'auth.getToken.request1', message: `Error: ${err}` });
          ...
          debugLogs.push({ function: 'auth.getToken.request2', message: `Error: ${responseBody.error}` });
          ...
          debugLogs.push({ function: 'auth.getToken.request3', message: `Error: ${responseBody.result.error}` });
          ...
          debugLogs.push({ function: 'auth.getToken.request4', message: `Success: ${responseBody}` });
