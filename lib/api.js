    ...
    if (error) {
      ...
      request(requestOptions, function(error, response, responseBody) {
        ...
        if (error) {
           ...
           * fire an "invalidateToken" event, which deletes the existing
           * token. Then try the call again, starting with auth.getToken()
           */
          ...
          err.message = reponseBody.result.error.detail;
