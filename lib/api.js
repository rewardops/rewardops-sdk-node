...
function apiCall(httpMethod, options, callback) {
  ...
  const { debugLogs = { push: () => {} } } = options;

  auth.getToken(options.config, (error, token) => {
    if (error) {
      debugLogs.push({ function: 'auth.getToken', message: `Error: ${JSON.stringify(error)}` });

      callback(error);
    } else {
      const requestOptions = {
        method: httpMethod,
        url: options.customPath ? options.customPath : urls.getApiBaseUrl() + options.path,
        json: true,
        headers: {
      ...
      request(requestOptions, (err, response, responseBody) => {
        if (err) {
          debugLogs.push({ function: 'auth.getToken.request1', message: `Error: ${JSON.stringify(err)}` });
          ...
          debugLogs.push({
            function: 'auth.getToken.request2',
            message: `Error: ${JSON.stringify(responseBody.error)}`,
          });

          error = getError(responseBody.error, response.statusCode, responseBody.result);
          callback(error);
          ...
          debugLogs.push({
            function: 'auth.getToken.request3',
            path: options.customPath || options.path,
            message: `Error: ${JSON.stringify(responseBody.result.error)}`,
          });

          error = getError(responseBody.result.error.detail, response.statusCode, responseBody.result);
          callback(error);
        } else {
          log(`Success: ${response.statusCode} ${urls.getApiBaseUrl()}${options.path}`);
          debugLogs.push({ function: 'auth.getToken.request4', message: `Success: ${JSON.stringify(responseBody)}` });
