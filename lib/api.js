  ...
  auth.getToken(options.config, (error, token) => {
    if (error) {
      ...
      const requestOptions = {
        method: httpMethod,
        url: urls.getApiBaseUrl() + options.path,
        json: true,
        headers: {
      ...
      log({
        level: 'info',
        message: `Request: ${httpMethod} ${urls.getApiBaseUrl()}${options.path}`,
      });
      ...
      request(requestOptions, (err, response, responseBody) => {
        if (err) {
          ...
          log(`API error: ${prettyPrint(responseBody.error)}`, { level: 'error' });
          ...
          log(`API error: ${prettyPrint(responseBody.result.error.detail)}`, { level: 'error' });

          error = getError(responseBody.result.error.detail, response.statusCode, responseBody.result);
          callback(error);
        } else {
          log.info(`Success: ${response.statusCode} ${urls.getApiBaseUrl()}${options.path}`);
