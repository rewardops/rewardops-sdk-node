...
const { ConfigurationError } = require('../utils/error');
...
const createOrder = ({ contextId }) => async (params, callback) => {
  try {
    const url = `${config.get('piiServerUrl')}/api/${config.get('apiVersion')}/programs/${contextId}/orders`;
...
const orderRecipientFactory = (contextTypeName, contextId, programCode) => {
  if (contextTypeName !== 'programs') {
    throw new ConfigurationError('Can only create an order recipient object for programs');
  }
  if (!config.get('piiServerUrl')) {
    throw new ConfigurationError('`piiServerUrl` is not configured');
  }
  if (!config.get('supportedLocales')) {
    throw new ConfigurationError('`supportedLocales` is not configured');
  }
