...
const getV5Token = callback =>
  request.post(
    {
      path: `${config.get('piiServerUrl')}/api/v5/auth/token`,
      params: {
...
const createOrder = ({ error, debugLogs: logs }, params, options, callback) => {
  if (error) {
    callback({ error, debugLogs: logs });
  }
  // TODO: consider adding validation see: createOrderInContext
  // see: lib/resources/orders.js
  api.post({ ...options, params, debugLogs }, (createOrderError, data, response) => {
    callback({ error: createOrderError, debugLogs }, data, response);
  });
};
...
const storeOrderRecipient = orderContext => async (requestBody, callback) => {
  ...
  debugLogs.push({ function: 'storeOrderRecipient', message: `pre-validation ${JSON.stringify(options)}` });
      ...
      getV5Token((error, response, body) => {
        ...
        request.post(
          ...
          (storeOrderRecipientError, piiData) =>
            createOrder({ error: storeOrderRecipientError, debugLogs }, { ...rest, ...piiData }, options, callback)
        );
      });
    })
