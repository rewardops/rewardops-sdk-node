...
const getAllProgramItems = programId => {
  return function getAll(params, callback) {
    /*
     * If 'favourites_only' or 'wishlist_only' is passed,
     * the SDK should call another endpoint to retrieve items.
     * Otherwise, the result will be fetched from the cache,
     * which may cause caching issues leading to outdated data.
     */
    const path =
      params.favourites_only === true || params.wishlist_only === true
        ? `/programs/${programId}/member_saved_items`
        : `/programs/${programId}/items`;
