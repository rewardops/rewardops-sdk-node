...
/**
 * Formats error messages from error object
 *
 * @param {*} dataInput error object with array properties containing error messages
 * @returns extracted error message object
 */
const formatErrorMessage = dataInput => {
  const keys = Object.keys(dataInput);
  const entries = keys.map(key => [
    key,
    Array.isArray(dataInput[key])
      ? dataInput[key].map(message => (typeof message === 'string' ? { message } : null)).filter(item => item !== null)
      : dataInput[key],
  ]);
  const result = {};
...
const filterLogData = data => {
  ...
  return isPlainObject(data)
    ? flow([rawData => redactSecrets(rawData), redactedData => redactyl.redact(formatErrorMessage(redactedData))])(data)
    : data;
};
