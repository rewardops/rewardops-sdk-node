...
const { isEmpty, isPlainObject, isArray, flow } = require('lodash');
...
const filterPIIData = (nestedDataQueue, data) => key => {
  // if the value is an object, add it to the queue
  if (typeof data[key] === 'object') {
    nestedDataQueue.push(data[key]);
    // else if the key is a PII attr, redact it
  } else if (PIIAttributes.includes(key)) {
    data[key] = REDACTED_MESSAGE;
  }
};

const filterLogData = data => {
  // guard against null, undefined and Error objects
  if (!isPlainObject(data)) {
    return data;
  }

  const nestedDataQueue = [data];
  /**
   * Traverses the data tree, redacting any sensitive data.
   *
   * NOTE: it mutates the data object.
   */
  while (nestedDataQueue.length > 0) {
    const node = nestedDataQueue.shift();
    // leverage `redact-secrets` for secrets, keys, etc.
    const redactedNode = redactSecrets(node);
    // manually check for PII
    Object.keys(redactedNode).map(filterPIIData(nestedDataQueue, node));
  }
  // returns the mutated/sanitized data object.
  return data;
};
