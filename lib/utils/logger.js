 ...
 *
 * @param {*} value Any value
 * @returns returns converted array of strings to object
 */
const convertArrayStringToObject = value => {
  value.forEach((val, index) => {
    if (typeof val === 'string') {
      value[index] = { message: val };
    }
  });
 ...
 *
 * @param {*} value Any value
 * @returns cleaned message object
 */
const cleanMessage = value => {
  Object.keys(value).forEach(key => {
    if (Array.isArray(value[key])) {
      convertArrayStringToObject(value[key]);
    }
  });
...
const filterLogData = data => {
  ...
  return isPlainObject(data)
    ? flow([rawData => redactSecrets(rawData), redactedData => redactyl.redact(cleanMessage(redactedData))])(data)
    : data;
};
