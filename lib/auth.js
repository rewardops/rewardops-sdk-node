...
function requestToken(requestOptions, retryStrategy, callback) {
  ...
  axios
    .request(axiosConfig)
    .then(response => {
      const { status: statusCode, headers, data: body } = response;
      const meta = {
      ...
      if (statusCode !== 200) {
        if (retryStrategy.attempts >= retryStrategy.maxAttempts) {
          ...
          if (headers['www-authenticate']) {
            const authHeaderMatch = headers['www-authenticate'].match(/error_description="(.*)"/);
            if (authHeaderMatch && authHeaderMatch[1]) {
              err.message += `${authHeaderMatch[1]} `;
            }
          }

          err.message += `(error ${statusCode})`;

          log(`Auth error: ${err.message}`, { level: 'error', meta });
    ...
    .catch(error => {
      ...
      if (error.code === 'ECONNABORTED' || error.message === 'ETIMEDOUT' || error.message === 'ESOCKETTIMEDOUT') {
        if (retryStrategy.attempts >= retryStrategy.maxAttempts) {
          /*
