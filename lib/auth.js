 ...
 * @param {{attempts: number, maxAttempts: number}} retryOptions Request retry options.
 * @param {module:api~requestCallback} callback Callback that handles the response.
 *
 * @protected
 */
function requestToken(requestOptions, retryOptions, callback) {
  ...
  request.post(requestOptions, (error, response, body) => {
    if (error && (error.message === 'ETIMEDOUT' || error.message === 'ESOCKETTIMEDOUT')) {
      if (retryOptions.attempts >= retryOptions.maxAttempts) {
        /*
        ...
        requestToken(requestOptions, retryOptions, callback);
      }
    } else if (error) {
      /*
