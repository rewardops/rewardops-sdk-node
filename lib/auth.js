  ...
  request.post(requestOptions, (error, response, body) => {
    if (error && (error.message === 'ETIMEDOUT' || error.message === 'ESOCKETTIMEDOUT')) {
      if (retryStrategy.attempts >= retryStrategy.maxAttempts) {
        ...
        // log(`Auth error: Reached maximum ${retryStrategy.maxAttempts} authentication retries`, { level: 'error' });
         ...
         */
        log(`Auth error: Retrying...`);
        // log(`Auth error: Retrying...`, { level: 'error' });
      ...
      // log(`Auth error: ${error}`, { level: 'error' });
      emitter.emit('unlockToken', error);
